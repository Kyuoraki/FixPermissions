...
foreach ($computer in  $hostsList) {
    ...
    if ( ($availabilityCheck.StatusCode) -eq 0 ) {
        ssh n.bobrov@$computer -p 33 powershell.exe -command "hostname;powershell -file 'C:\Program Files\OpenSSH\FixHostFilePermissions.ps1';powershell -file 'C:\Program Files\OpenSSH\FixUserFilePermissions.ps1'" | Out-File "C:\Fix\log.txt" -Force
    }else{
